...
class Bottleneck(nn.Module):#Bottleneck是残差块的类型，用于构建 ResNet 模型中的每个残差层
    expansion = 4
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_classes=3, num_classes=1000, zero_init_residual=False):#in_classes默认为3，通常用于RGB彩色图像
        #注意，num_classes是分类个数，需要在main中再调整，比如model.fc = nn.Linear(512, 2)，表示二分类
        super(ResNet, self).__init__()
        self.inplanes = 64
        self.in_classes = in_classes
        self.conv1 = nn.Conv2d(self.in_classes, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
...
def resnet18(pretrained=False, **kwargs):
    ...
    """
    model = ResNet(BasicBlock, [2, 2, 2, 2], **kwargs)#[2,2,2,2]：这是一个列表，指定了 ResNet 中每个残差层中残差块的数量。
    if pretrained:
...
def resnet50(pretrained=False, **kwargs): #49个卷积层+1个全连接层
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3], in_classes=3,**kwargs)
    if pretrained:
